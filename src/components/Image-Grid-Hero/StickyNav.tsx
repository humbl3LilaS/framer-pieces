...
export default function StickyNav({bgTransition, textTransition}: { bgTransition: MotionValue<string> , textTransition: MotionValue<string>}) {
    ...
    return (
        <header>
            <motion.div className={"w-full h-16 p-4  fixed top-0 left-0 flex  justify-between items-center text-white"}
                        style={{background: bgTransition}}

            >
                <motion.h1 className={"text-cyan-900"} style={{color: textTransition}}>
                    Edelweiss
                </motion.h1>
